...
export const fatchAllUsers = createAsyncThunk(
  ...
  async (data: UsersAll)=> {
    ...
    return {users:response.data.data[0] ,count:response.data.data[1].count.count};
  }
);
...
export const fatchDeleteUsers = createAsyncThunk(
  ...
  async (data: Delete) => {
    ...
    const response = await data.axiosPrivate.get(`${BASE_URL}/users?page=${data.page}&pageSize=${data.pageSize}`);
    return {users:response.data.data[0] ,count:response.data.data[1].count.count};
  }
);
...
export const fatchUpdateUsers = createAsyncThunk(
  ...
  async (data: Update) => {
    const response = await data.axiosPrivate.put(`${BASE_URL}/users/${data.id}`,data.data);
...
export const fatchInsertUsers = createAsyncThunk(
  ...
  async (data: Insert) => {
    const response = await data.axiosPrivate.post(`${BASE_URL}/users`,data.data);
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(
        ...
        (
          state,
          action: PayloadAction<{ users: Users[]; count: number }>
        ) => {
          state.isLoading = false;
      ...
      .addCase(fatchDeleteUsers.fulfilled, (state, 
        action: PayloadAction<{ users: Users[]; count: number }>
        ) => {
        state.isLoading = false;
        state.ErrorMessage = "";
      ...
      .addCase(fatchInsertUsers.fulfilled, (state, action:PayloadAction<Users>) => {
        state.isLoading = false;
        state.ErrorMessage = "";
        state.insert = 201;
      ...
      .addCase(fatchUpdateUsers.fulfilled, (state, 
